...
export function slugify(string: string, options?: SlugifyOptions | string): string {
  if (typeof string !== "string") {
    throw new Error("slugify: string argument expected");
  }

  const opts: SlugifyOptions = typeof options === "string" ? { replacement: options } : options || {};

  const locale = opts.locale ? locales[opts.locale] || {} : {};
  ...
  let slug = string
    ...
    .reduce((result, ch) => {
      ...
      if (appendChar === undefined) appendChar = charMap[ch as keyof typeof locale];
